...
void main (void)
{
    ...
    float height = texture2D(bump_texture, srcUV).r*2.0-1.0;
    ...
    if(NdotL > 0.0)
    {
		...
		vec3 H = normalize(eye + L); // pùl vektor
		float NdotH = dot(N, H);
		if(NdotH > 0.0) specular = gl_LightSource[0].specular * gl_FrontMaterial.specular * pow(NdotH, gl_FrontMaterial.shininess);
    }
